...
// 后台查询接口
goodsAdmin.post('/getSearchGoods',async(req,res)=>{
    ...
    db.query(sql,[content,(searchPage-1)*pageSize,pageSize],(err,results)=>{
        if(err) return console.log(err.message);
        // console.log(results);
        res.render("goodsSearch",{goodsData:results,userId:req.query.userId,userType:req.query.userType,searchPage:searchPage,total:total,searchContent:content})
    })
})


goodsAdmin.get('/getSearchGoods',async(req,res)=>{
